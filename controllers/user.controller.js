...
class UserController {
  ...
  getCartsByUserId = async (req, res) => {
    ...
    try{
      const userCarts = await Carts.find({user_id: req.params.user_id}.populate({_id:cart_id}));
  ...
  createUser = async (req, res) => {
    ...
    try{
      const newUser = await Users.create({name,last_name,email,password,address})
